...
public class ProxyScheduleCommand extends CommandBase {
    private final Set<com.arcrobotics.ftclib.command.Command> m_toSchedule;
     ...
     */
    public ProxyScheduleCommand(com.arcrobotics.ftclib.command.Command... toSchedule) {
        m_toSchedule = new HashSet<com.arcrobotics.ftclib.command.Command>(Arrays.asList(toSchedule));
    }

    @Override
    public void initialize() {
        for (com.arcrobotics.ftclib.command.Command command : m_toSchedule) {
            command.schedule();
        }
    }
