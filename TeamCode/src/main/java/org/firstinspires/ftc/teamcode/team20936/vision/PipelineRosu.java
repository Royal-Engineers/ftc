...
public class PipelineRosu extends OpenCvPipeline {
    ...
    private final int ConeDistance = 25;

    private ArrayList<Pair<Point, Point>> ConeList = new ArrayList<>();
    public PipelineRosu(Telemetry t){telemetry = t;}
       ...
       /* Mat edges = new Mat();
        ...
        for ( int i = 1; i < m_mat.cols() - 16; ++i )
        {
            ...
            if ( val > BoundDensity ) {
                ...
                boolean Ebun;
                if ( RightBound != -1 && LeftBound != -1 && UpperBound != -1 && LowerBound != -1 && RightBound - LeftBound > 5 )
