...
public class commandBaseTeleOp extends CommandOpMode{
    ...
    public void initialize() {
        ...
        armRev = hardwareMap.get(DcMotorEx.class, "rev_hd_brat");
       ...
       controller1.getGamepadButton(GamepadKeys.Button.DPAD_LEFT)
               .toggleWhenPressed(new clawGrab(m_intakeSubsystem), new clawRelease(m_intakeSubsystem));
    }
    ...
    public void run() {
        ...
        if (gamepad1.right_trigger > 0.01 && m_depositSubsystem.getPosition()<=2000)
            CommandScheduler.getInstance().schedule(
                    (new manualControl(m_depositSubsystem, m_depositSubsystem.getPosition() + 30 * (Math.abs((int) gamepad1.right_trigger))))
            );
        else if (gamepad1.left_trigger > 0.01 ) {
                CommandScheduler.getInstance().schedule(
                        (new manualControl(m_depositSubsystem, m_depositSubsystem.getPosition() - 30 * (Math.abs((int) gamepad1.left_trigger))))
                );
        }
