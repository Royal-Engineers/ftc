...
public class DriveSubsystem {
    ...
    public void setKinematics(int cnt)
    {
        ...
        rx[cnt]=Math.cos(beta[cnt])*R;
        ry[cnt]=Math.sin(beta[cnt])*R;
        ...
        if(thetam[cnt]>180) thetam[cnt]=thetam[cnt]-360;
    }
    public void UpdateGamepad(){
        ...
        //probabil field centric
        botHeading =-imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS);
        double aux =vy*Math.cos(botHeading)+vx*Math.sin(botHeading);
        vx =-vy*Math.sin(botHeading)+vx*Math.cos(botHeading); vy=aux;
        //doar probabil
